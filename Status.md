...
- **Web (PWA):** Live at https://salmon-mud-07f7aa310.7.azurestaticapps.net on Azure Static Web Apps, with auto-deploy on push to `main`. Works fully offline after first load via service worker (cache `usfs-collector-v1.14`).
...
- **Deployed 2026-09-23 (web), second pass from a full code review:** exports no longer delete backup photo copies of entries they didn't export; a reload in the middle of an edit resumes edit mode instead of creating a duplicate entry; the offline cache never replaces good files with error pages or redirects, and now includes the app manifest; citation search shows a clear message instead of retrying endlessly when its data can't load; Save & New while a photo is still processing can no longer drop the photo into the wrong entry; choosing a forest or saving photo settings no longer breaks when storage is full; the saved list escapes entry text; storage warnings track the real limits (including the ~5 MB entry store that fills first on iPads); the save toast reports the true photo count. Cache `usfs-collector-v1.14`.
